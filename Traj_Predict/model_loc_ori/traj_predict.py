...
class Predict(object):
    ...
    def __init__(self,
        ...
        self._encoder_feature_size = 4  # bbox
        self._encoder_feature_size_ori = 4  # body-head degree
        self._encoder_feature_size_flow = 146
    ...
    def get_data_helper(self, data, data_type):
        ...
        for dt in data_type:
            if dt == 'image':
                continue
            if dt == 'ped_op_flow' or dt == 'ego_op_flow':
                data[dt] = [np.array(sublist) for sublist in data[dt]]
                data[dt] = np.array(data[dt]).reshape(np.array(data[dt]).shape[:-2]+(-1,))
                ego_ped_shape = data[dt].shape[0]
                data[dt] = [data[dt][i, :, :] for i in range(ego_ped_shape)]
    ...
    def data_split(self, data, index):
        # 只有使用交叉验证时才用到，如果报错，优先看这里
        image1, pid1, bbox1, speed1, tte1, deg, ego, ped = [], [], [], [], [], [], [], []
        ...
        for n1 in index:
            ...
            if 'degree_pred' in data.keys():
                deg.append(data['degree_pred'][n1])
            if 'ego_op_flow' in data.keys():
                ego.append(data['ego_op_flow'][n1])
                ped.append(data['ped_op_flow'][n1])
        ...
        if 'ego_op_flow' in data.keys():
            data_['ego_op_flow'] = ego
            data_['ped_op_flow'] = ped
    ...
    def get_data(self, data, index=None, **model_opts):
        ...
        data_types = set(opts['enc_input_type1'] + opts['enc_input_type2'] + opts['enc_input_type3'] +
                         opts['dec_input_type'] + opts['prediction_type'])
        ...
        enc_input_bbox = self.get_data_helper(obs_slices, opts['enc_input_type1'])
        enc_input_ori = []
        enc_input_flow = []
        if 'degree_pred' in opts['enc_input_type2']:
            enc_input_ori = self.get_data_helper(obs_slices, opts['enc_input_type2'])
        if 'ped_op_flow' in opts['enc_input_type2'] and 'degree_pred' not in opts['enc_input_type2']:
            enc_input_ori = self.get_data_helper(obs_slices, opts['enc_input_type2'])
        if 'ped_op_flow' in opts['enc_input_type3']:
            enc_input_flow = self.get_data_helper(obs_slices, opts['enc_input_type3'])
        ...
        if not len(enc_input_ori) > 0:
            enc_input_ori = np.zeros(shape=enc_input_bbox.shape)

        if not len(enc_input_flow) > 0:
            enc_input_flow = np.zeros((enc_input_bbox.shape[0], enc_input_bbox.shape[1], 146))
        ...
        if 'TTE' in obs_slices.keys():
            return {'obs_image': obs_slices['image'],
                    ...
                    'enc_input1': enc_input_bbox,
                    'enc_input2': enc_input_ori,
                    'enc_input3': enc_input_flow,
                    'dec_input': dec_input,
    ...
    def get_path(self,
        ...
        用于保存模型和配置数据的路径生成器方法。它可以根据需要创建目录。
        :param file_name: The actual save file name , e.g. 'model.h5'  实际保存的文件名。“model.h5”
        :param save_folder: The name of folder containing the saved files  保存文件的文件夹名
        :param dataset: The name of the dataset used  使用的数据集的名称
        :param save_root_folder: The root folder  根文件夹
        :return: The full path for the model name and the path to the final folder  模型名称的完整路径和最终文件夹的路径
        """
    ...
    def log_configs(self, config_path, batch_size, epochs,
        ...
        with open(config_path, 'wt') as fid:
            ...
            fid.write("%s: %s\n" % ('encoder_feature_size_bbox', str(self._encoder_feature_size)))
            fid.write("%s: %s\n" % ('encoder_feature_size_ori', str(self._encoder_feature_size_ori)))
    ...
    def train(self, data_train, data_val,
        ...
        if val:
            ...
            print("Number of samples:\n Train: %d \n Val: %d \n"
                  % (train_data['enc_input1'].shape[0], val_data['enc_input1'].shape[0]))
        else:
            print("Number of samples:\n Train: %d \n"
                  % (train_data['enc_input1'].shape[0]))

        self._observe_length = train_data['enc_input1'].shape[1]
        self._predict_length = train_data['pred_target'].shape[1]

        self._encoder_feature_size = train_data['enc_input1'].shape[2]
        self._encoder_feature_size_ori = train_data['enc_input2'].shape[2]
        self._encoder_feature_size_flow = train_data['enc_input3'].shape[2]
    ...
    def test(self, data_test, model_path=''):
        ...
        if model_opts['prediction_type'][0] == 'bbox':
            ...
            test_data = self.get_data(data_test, **model_opts)
            test_obs_data_org = [test_data['enc_input1'], test_data['enc_input2'],
                                 test_data['enc_input3'], test_data['dec_input']]#这里有一点点的奇怪，应该是没有test_data['enc_input2']和test_data['enc_input3']的
    ...
    def test_(self, data_test, model_path=''):
        ...
        perf['mse'] = performance.mean(axis=None) #axis=None表示所有元素一起求均值
        perf['mse_last'] = performance[:, -1, :].mean(axis=None)
        if model_opts['predict_length'] == 45:
            perf['mse-15'] = performance[:, 0:15, :].mean(axis=None)
            perf['mse-30'] = performance[:, 0:30, :].mean(axis=None)
        ...
        if model_opts['prediction_type'][0] == 'bbox':
            ...
            for b in range(test_results.shape[0]):
                for s in range(test_results.shape[1]):
                    centers[b, s, 0] = (test_target_data_org[b, s, 2] + test_target_data_org[b, s, 0]) / 2#真实边界框中心点
                    centers[b, s, 1] = (test_target_data_org[b, s, 3] + test_target_data_org[b, s, 1]) / 2
                    res_centers[b, s, 0] = (test_results[b, s, 2] + test_results[b, s, 0]) / 2#预测的边界框中心点
            ...
            perf['center_mse_last'] = c_performance[:, -1, :].mean(axis=None)
            if model_opts['predict_length'] == 45:
                perf['cmse-15'] = c_performance[:, 0:15, :].mean(axis=None)
                perf['cmse-30'] = c_performance[:, 0:30, :].mean(axis=None)
    ...
    def test_loc_speed(self, data_test, traj_model_path='', speed_model_path=''):
        ...
        # speed
        test_results = box_speed_model.predict([box_data['enc_input1'], box_data['enc_input2'],
                                               box_data['enc_input3'], speed_results],
        ...
        for j in range(len(test_results[0])):
            if j == 0:
                test_results[:, j, :] = test_results[:, j, :] + test_obs_data_org[0][:, -1, 0:4] #预测的是帧间差值，所以相加
            else:
                test_results[:, j, :] = test_results[:, j, :] + test_results[:, j - 1, :]

        # Performance measures for bounding boxes
        perf = {}
        performance = np.square(test_results - test_target_data_org)#求平方
        perf['mse-15'] = performance[:, 0:15, :].mean(axis=None)
    ...
    def pie_encdec(self):
        ...
        _encoder_input1 = Input(shape=(self._observe_length, self._encoder_feature_size),
                                name='encoder_input1')
        _encoder_input2 = Input(shape=(self._observe_length, self._encoder_feature_size_ori),
                                name='encoder_input2')
        _encoder_input3 = Input(shape=(self._observe_length, self._encoder_feature_size_flow),
                                name='encoder_input3')

        # Temporal attention module
        _attention_net1 = self.attention_temporal(_encoder_input1, self._observe_length)
        _attention_net2 = self.attention_temporal(_encoder_input2, self._observe_length)
        _attention_net3 = self.attention_temporal(_encoder_input3, self._observe_length)

        # Generate Encoder LSTM Unit
        encoder_model1 = self.create_lstm_model(name='encoder_network1')
        _encoder_outputs_states1 = encoder_model1(_attention_net1)
        _encoder_states1 = _encoder_outputs_states1[1:]

        encoder_model2 = self.create_lstm_model(name='encoder_network2')
        _encoder_outputs_states2 = encoder_model2(_attention_net2)
        _encoder_states2 = _encoder_outputs_states2[1:]

        encoder_model3 = self.create_lstm_model(name='encoder_network3')
        _encoder_outputs_states3 = encoder_model3(_attention_net3)
        _encoder_states3 = _encoder_outputs_states3[1:]

        # Generate Decoder LSTM unit
        decoder_model = self.create_lstm_model(name='decoder_network', r_state=False)
        _hidden_input1 = RepeatVector(self._predict_length)(_encoder_states1[0])
        _hidden_input2 = RepeatVector(self._predict_length)(_encoder_states2[0])
        _hidden_input3 = RepeatVector(self._predict_length)(_encoder_states3[0])
        ...
        _embedded_hidden_input1 = Dense(self._embed_size, activation='relu')(_hidden_input1)
        _embedded_hidden_input1 = Dropout(self._embed_dropout,
                                          name='dropout_dec_input1')(_embedded_hidden_input1)

        _embedded_hidden_input2 = Dense(self._embed_size, activation='relu')(_hidden_input2)
        _embedded_hidden_input2 = Dropout(self._embed_dropout,
                                          name='dropout_dec_input2')(_embedded_hidden_input2)

        _embedded_hidden_input3 = Dense(self._embed_size, activation='relu')(_hidden_input3)
        _embedded_hidden_input3 = Dropout(self._embed_dropout,
                                          name='dropout_dec_input3')(_embedded_hidden_input3)

        decoder_concat_inputs = Concatenate(axis=2)([_embedded_hidden_input1, _embedded_hidden_input2,
                                                     _embedded_hidden_input3, _decoder_input])

        # Self attention unit
        att_input_dim = self._embed_size*3 + self._decoder_feature_size
        ...
        decoder_output = decoder_model(decoder_concat_inputs,
                                       initial_state=_encoder_states1)#initial_state是解码器的初始隐藏状态，通常由编码器的最终隐藏状态传递给解码器，这里的隐藏状态是轨迹信息
        decoder_output = Dense(self._prediction_size,
                               activation='linear',
                               name='decoder_dense')(decoder_output)

        net_model = Model(inputs=[_encoder_input1, _encoder_input2, _encoder_input3, _decoder_input],

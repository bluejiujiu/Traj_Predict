...
def train(train_data, val_data,
        ...
        print("Number of samples:\n Train: %d \n Val: %d \n"
              % (len(train_data['enc_input']), len(val_data['enc_input'])))

        t._observe_length = train_data['enc_input'].shape[1]
        t._predict_length = train_data['pred_target'].shape[1]

        t._encoder_feature_size = train_data['enc_input'].shape[2]
...
def train_test_daimler(train_test=2,
    ...
    traj_model_opts = {'normalize_bbox': True,
                       ...
                       'observe_length': 8,
                       'predict_length': 8,
                       'enc_input_type': ['bbox', 'degree_pred'],
    ...
    if train_test == 1:
        ...
        #param = [128]
        for p in param:
            traj = PIEPredict(num_hidden_units=p)
...
def main(train_test=2):
    out_model_path = 'model/daimler/trajectory/loc_offset_ori_speed/'
